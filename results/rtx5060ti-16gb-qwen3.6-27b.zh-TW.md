...
的條件下再次確認。35,072 能載入、18-token 的暖身 prompt 能全速生成，
然後在一個 **64 token** 的 prompt 下 CUDA OOM，留下一個殭屍程序。

這些測試早於 prompt 階梯，所以當時全程都是用灌滿視窗的 prompt 認證的。
階梯是在確認「64 個 token 一樣能可靠地區分 34,816 與 35,072」之後才建立的 ——
這個論據從來就不是為了**長** prompt，而是為了「比暖身 prompt 更大的」prompt。
...
**llama.cpp 的 buffer 配置是完全靜態的。** 所有東西都在載入時配置完畢，
推論過程中不會成長。

這正是 35,072 那次失敗長期被誤讀的原因：如果什麼都不會成長，
那「負載下崩潰」看起來就是不可能的。取樣器看不見的是
**kernel 程式碼在第一次被觸碰時載入 device memory** ——
那次配置**正因為失敗了**所以從來不會進入 `nvidia-smi` 的用量，
而且就算成功，它的量也在 1 MiB 解析度以下。

所以正確的說法比表面上窄。在 `LAZY` 之下，一個通過這裡所有 prompt 的配置，
仍然可能之後因為碰到還沒碰過的 kernel 而 OOM ——
換個 sampler、加個 grammar、換個 batch 形狀都算。
在 `EAGER` 之下、所有 kernel 從啟動就常駐時，「靜態配置」才真的等於它看起來的意思：
那點餘裕只需要撐住「其他程序來動這張卡」。
