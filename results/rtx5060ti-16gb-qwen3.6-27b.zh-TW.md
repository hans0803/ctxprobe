...
5060 Ti 有 8GB 和 16GB 兩個版本，出自同一顆 GB206 die ——
一樣的 4608 個 CUDA 核心、一樣的時脈、一樣的 128-bit GDDR7 448 GB/s。
規格表上唯一的差別就是容量，所以把 16GB 卡限制在 8GB 的預算下，
量出來的速度可以直接對應到真實的 8GB 版本。

方法：用另一個 CUDA 程序佔住 7899 MiB，剩下 7799 MiB。
真實 8GB 卡的可配置預算大約是 7800 MiB ——
它的驅動保留區比這張卡的 462 MiB 小，所以這是估計中偏保守的一側。
...
跟同一個模型完整放在 16GB 卡上、在相近 context 下對比：
...
**把 65 層中的 37 層溢出到系統記憶體，代價是大約 80% 的生成速度。**
每個 token 都得跨過 PCIe 去找住在 DDR4 裡的那些層，
再強的 CPU 也不可能讓這件事跟顯存競爭。

這正是 [model-quant.zh-TW.md](../docs/model-quant.zh-TW.md) 那條規則的具體案例：
「**完整**裝得下的最大量化」勝過「會溢出的更好量化」。
在 8GB 卡上，27B 的答案不是換更小的量化 ——
`IQ3_XXS` 仍然有 11.99 GB，一樣裝不下。答案是換更小的模型。

注意這一節的峰值顯存數字包含了模擬程序佔住的 7899 MiB，
要看模型自己的用量請減掉這部分。
