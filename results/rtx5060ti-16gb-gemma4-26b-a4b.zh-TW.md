...
實測於 2026-08-11；8GB 那一節於 2026-09-11 重新量測。與 [Qwen3.6-27B 那輪](rtx5060ti-16gb-qwen3.6-27b.zh-TW.md)
...
騰出這麼多顯存之後，這張 8GB 卡跑得動模型**完整的 262,144 context** ——
那是 Gemma 4 的訓練上限，不是記憶體邊界：

| Context | 結果 | 峰值顯存 | 模型自己的 | Prefill | 生成 | 填充 |
|---|---|---|---|---|---|---|
| 131,072 | PASS | 12,168 MiB | 4,117 MiB | 700 tok/s | 38.6 tok/s | 4,038 |
| 262,144 | PASS | 14,168 MiB | 6,117 MiB | 702 tok/s | 39.0 tok/s | 4,038 |
| **262,144** | **95% 填充 PASS** | 14,168 MiB | 6,117 MiB | **501 tok/s** | **17.95 tok/s** | **249,013** |

「模型自己的」是扣掉佔位程序及其 CUDA context 的 8,051 MiB。滿視窗時模型用掉
7.8 GB 預算中的 6.1 GB，所以當模型自己的 context 用盡時還剩 1.7 GB。
一個 26B 的模型，在 8GB 上，跑 256K —— 還有餘裕。249K 個 token 的 prompt 花了 497 秒，
在 ctxprobe 900 秒的地板之內，時間預算完全沒有介入。

滿視窗生成 17.95 tok/s，空視窗是 39。形狀跟 16GB 那輪的 120 → 50.8 一樣：
五層全域注意力仍要掃過整個 cache，其餘 25 層則停在滑動視窗。
這裡 KV 每 131,072 個 token 長 2,000 MiB —— 每 1K 15.3 MiB，
是同樣 30 層每層都用全域注意力時所需的六分之一。

八月那輪對這個配置回報的是 131,072。那是當時 ctxprobe 的搜尋上限，不是對這張卡的量測；
上限現在是 262,144，這一節就是重新量測的結果。
...
`--max` 來自 GGUF metadata（262144）。8GB 那一節先跑 `python tools/hold-vram.py 7800 &`，
並在結尾加上 `-- --cpu-moe`。
